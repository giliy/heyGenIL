...
def main():
    ...
    events = []
    missing = set()       # committed-tier misses -> hard fail
    missing_pro = set()   # gitignored pro/ clips absent on this machine -> warn + skip
    for e in plan["events"]:
        ...
        if not os.path.exists(f):
            # pro/ clips are gitignored (Sonniss/99Sounds licenses forbid committing them);
            # skip gracefully and point at the fetch script rather than aborting the whole mix.
            if clip["file"].startswith("pro/"):
                missing_pro.add(e["sfx_id"])
            else:
    ...
    if missing_pro:
        print("  SKIPPED pro/ clips not on this machine (run tools/fetch_pro_sfx.py):",
              ", ".join(sorted(missing_pro)))
    if flags["print"]:
        return
    if not events:
        sys.exit("no events to mix." +
                 (" (all remaining cues were gitignored pro/ clips — fetch them with "
                  "tools/fetch_pro_sfx.py)" if missing_pro else ""))
    if missing:
    ...
    cmd = [ffw.path(), "-y", "-hide_banner", "-i", preview]
    for e in events:
        cmd += ["-i", e["file"]]
    ...
    # NOTE: needs a full ffmpeg — the Remotion-bundled build (--disable-filters)
    # lacks alimiter/sidechaincompress. Use a full build (e.g. gyan.dev) for duck.
    ...
    print(f"\nrendering -> {show(out)}")
    run(cmd)
    dur = run([ffw.ffprobe_path(), "-v", "error", "-show_entries", "format=duration",
