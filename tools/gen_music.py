...
Usage:
  ...
  --engine elevenlabs|acestep   music generator (default: elevenlabs). `acestep` calls
                               ACE-Step on fal (fal-ai/ace-step, Apache-2.0, ~$0.0002/s of
                               audio) instead of the ElevenLabs Music API (~15x cheaper).
                               The normalize -> probe -> catalog pipeline and the catalog
                               schema are identical for both engines; only `source` differs.

  P2 #13 music strategy (see docs/music-generation.md): SELF-HOSTING ACE-Step is hardware-blocked
  on this machine (torch is CPU-only, integrated Intel GPU, no published CPU times). The working
  $0-marginal-equivalent is: ACE-Step via fal for NON-PREMIUM beds (~$0.006/30s, Apache-2.0), and
  tools/import_music.py (Pixabay, #17) for PREMIUM human ad beds. Prefer `--engine acestep` for any
  non-premium bed; reserve elevenlabs for the paid ceiling only.

Needs ELEVENLABS_API_KEY (engine=elevenlabs) or FAL_KEY (engine=acestep) in .env
(see .env.example). ffmpeg/ffprobe on PATH. fal-client must be installed into the
running interpreter for engine=acestep (`pip install fal-client`).
"""
import json
import os
import re
...
# --- ACE-Step on fal (engine=acestep) -----------------------------------------------
ACESTEP_ENDPOINT = "fal-ai/ace-step"      # verified live + priced via fal pricing API
ACESTEP_USD_PER_SEC = 0.0002              # $/second of generated audio (fal pricing API)
ACESTEP_MAX_S = 190.0                     # ACE-Step caps a single generation < ~3m10s
...
LICENSE_ACESTEP = ("ACE-Step generated via fal (Apache-2.0, fully commercial, no attribution, "
                   "no revenue cap)")
SOURCE_ACESTEP = "fal:ace-step"
...
def find_audio_url(obj):
    """Walk any fal response schema for the first audio-looking url."""
    if isinstance(obj, dict):
        if isinstance(obj.get("url"), str) and "audio" in str(obj.get("content_type", "")):
            return obj["url"]
        for k in ("audio", "audio_url", "audio_file"):
            v = obj.get(k)
            ...
            if isinstance(v, str) and v.startswith(("http", "data:")):
                return v
        u = obj.get("url")
        if isinstance(u, str) and any(x in u for x in (".mp3", ".wav", ".m4a", ".ogg", ".flac")):
            return u
        for v in obj.values():
...
def generate_acestep(fal_key, prompt, duration_s, seed=None):
    """ACE-Step (fal-ai/ace-step) -> mp3 bytes. Instrumental via lyrics='[inst]' (ACE-Step
    treats [inst]/[instrumental] as 'no vocals'). Uses fal-client; FAL_KEY set from .env."""
    try:
        import fal_client
    except ImportError:
        sys.exit("fal-client not installed in this interpreter — run: pip install fal-client")
    os.environ["FAL_KEY"] = fal_key
    args = {"tags": prompt, "lyrics": "[inst]", "duration": float(max(1, round(duration_s)))}
    if seed is not None:
        args["seed"] = int(seed)
    print(f"   acestep payload: tags=\"{prompt[:70]}...\"  duration={args['duration']}s")
    result = fal_client.subscribe(ACESTEP_ENDPOINT, arguments=args, with_logs=False)
    url = find_audio_url(result)
    if not url:
        sys.exit("ace-step returned no audio url:\n" + json.dumps(result)[:500])
    req = urllib.request.Request(url, headers={"User-Agent": "gen_music.py"})
    with urllib.request.urlopen(req, timeout=300) as resp:
        return resp.read()
...
def main():
    ...
    engine = "elevenlabs"
    if "--engine" in args:
        engine = args[args.index("--engine") + 1].strip().lower()
    if engine not in ("elevenlabs", "acestep"):
        sys.exit(f"unknown --engine '{engine}' (expected elevenlabs|acestep)")
    ...
    fal_key = env.get("FAL_KEY", "").strip()

    # engine-specific catalog metadata; schema identical, only these three fields differ
    if engine == "acestep":
        eng_model, eng_source, eng_license = ACESTEP_ENDPOINT, SOURCE_ACESTEP, LICENSE_ACESTEP
    else:
        eng_model, eng_source, eng_license = model, SOURCE, LICENSE
    ...
    def entry(s, rel, path, length_ms):
        ...
        return {
            ...
            "peak_dbfs": peak, "loudness_lufs": lufs, "source": eng_source, "model": eng_model,
            "force_instrumental": force_instrumental, "license": eng_license,
    ...
    if renorm:  # re-balance existing beds to the current loudness target, no API
        ...
        for s in palette["beds"]:
            ...
            if not os.path.exists(path):
                continue
            e = entry(s, rel, path, by_id.get(s["id"], {}).get("requested_ms"))
            old = by_id.get(s["id"], {})
            if old.get("source"):  # never clobber how a bed was actually generated
                e["source"], e["model"], e["license"] = old["source"], old.get("model"), old.get("license")
            by_id[s["id"]] = e
            n += 1
    ...
    print(f"palette: {len(palette['beds'])} beds | engine: {engine} | to generate: {len(todo)} | skip (exist): {len(skip)}")
    if skip:
    ...
    if dry:
        for s, rel, _, length_ms in todo:
            extra = ""
            if engine == "acestep":
                extra = f"  ~${(min(length_ms/1000, ACESTEP_MAX_S))*ACESTEP_USD_PER_SEC:.4f} (acestep)"
            print(f"  WOULD generate {s['id']} -> {rel}  ({length_ms/1000:.0f}s)  \"{s['prompt'][:60]}...\"{extra}")
        return
    if engine == "elevenlabs" and todo and not api_key:
        sys.exit("ELEVENLABS_API_KEY not set in .env — cannot generate. (Add it, or use --dry-run.)")
    if engine == "acestep" and todo and not fal_key:
        sys.exit("FAL_KEY not set in .env — cannot generate via ACE-Step. (Add it, or use --dry-run.)")

    for s, rel, path, length_ms in todo:
        if engine == "acestep":
            duration_s = min(length_ms / 1000.0, ACESTEP_MAX_S)
            cost = duration_s * ACESTEP_USD_PER_SEC
            print(f"\n-> {s['id']}  ({duration_s:.0f}s via acestep)  derived cost ${cost:.4f}")
            audio = generate_acestep(fal_key, s["prompt"], duration_s)
            length_ms = int(round(duration_s * 1000))
        else:
