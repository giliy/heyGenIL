...
Methods (--method, default auto):
  rembg — ML matting via rembg (default model birefnet-general / BiRefNet, MIT-licensed).
          Handles cast shadows, gray backgrounds, subjects with white areas. PREFERRED;
          auto-picked when rembg is installed in the venv. NOTE: first run downloads the
          BiRefNet weights (~900MB) to rembg's model cache.
...
Usage:
  ...
  python tools/cutout.py in.png out.png --method rembg --model birefnet-general
...
def matte_rembg(img: Image.Image, model: str = "birefnet-general") -> Image.Image:
    """ML matte via rembg — the robust path for shadows and non-white backgrounds.

    Uses the ``birefnet-general`` (BiRefNet) session by default — MIT-licensed (rembg's
    default ``bria-rmbg`` is non-commercial) and produces cleaner die-cut edges. The session
    is created once and reused. NOTE: the first run downloads the BiRefNet weights (~900MB)
    to rembg's model cache. Pass a different model name (e.g. ``u2net``) to override.
    """
    global _SESSION
    ...
    cut = remove(img.convert("RGBA"), session=_SESSION)
...
def main():
    ...
    ap.add_argument("--model", default="birefnet-general",
                    help="rembg session/model to use with --method rembg (default: birefnet-general)")
    ...
    if method == "rembg":
        matte = matte_rembg(img, model=args.model)
    else:
        try:
