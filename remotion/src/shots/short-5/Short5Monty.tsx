...
import { GlowReveal, SceneTransition, sceneCut, ShortsBackdrop } from '../../lib/polish';
...
const RevealScene: React.FC = () => {
  ...
  return (
    <FadeIn>
      ...
      {/* "×2" badge on the double beat — T07 glow/drop-shadow reveal */}
      {doubleP > 0.01 ? (
        <GlowReveal
          progress={doubleP}
          color={TEAL}
...
const Short5Monty: React.FC = () => (
  <AbsoluteFill style={{ background: '#0e1119' }}>
    {/* T07: animated brand-mesh backdrop (indigo->violet->teal) + subtle grain */}
    <ShortsBackdrop base="#0e1119" intensity={0.5} grain={0.04} />
    {/* HOOK — frame 0 is the thumbnail; a plain Sequence so the loop seam is untouched */}
    <Sequence from={0} durationInFrames={102}>
      <HookScene mode="settle" />
    </Sequence>
    {/*
      INTERIOR cuts use SceneTransition. Durations are padded +24 (the transition
      length) so each interior scene still STARTS at its original global anchor —
      Setup@102, Quiz@399, Reveal@495, Proof@954 — keeping every cue synced to the
      VO while the cut itself crossfades. Packed span = 102..1200 exactly.
      NO transition on the loop seam (Hook scenes) per the CLAUDE.md loop contract.
    */}
    <Sequence from={102} durationInFrames={1098}>
      <SceneTransition>
        ...
        {sceneCut({kind: 'soft'})}
        <TransitionSeries.Sequence durationInFrames={483}>
          <RevealScene />
        </TransitionSeries.Sequence>
        {sceneCut({kind: 'hard'})}
        <TransitionSeries.Sequence durationInFrames={246}>
          <ProofScene />
        </TransitionSeries.Sequence>
      </SceneTransition>
    </Sequence>
    {/* LOOP TAIL — plain Sequence ending at the last frame to rhyme with frame 0 */}
    <Sequence from={1200} durationInFrames={90}>
      <FadeIn dur={12}>
        <HookScene mode="grow" />
      </FadeIn>
    </Sequence>
    <Captions lines={VO} mode="pill" y={1310} accent={GOLD} />
