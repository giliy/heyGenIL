...
7. **Coverage / bleed rule (locked 2026-08-21, from vox-2 QA):** a full-bleed background
   layer MUST be sized to cover the board at the camera's **max zoom-out** (z=1.0), not just
   at the settled frame — otherwise a static background reads "stuck" and any layer sitting
   past its edge overflows onto the board. When generating the paper/wall base, prompt and
   aspect it for the widest camera state; when placing layers, keep them inside the base's
   bleed. QA check: READ the frame at the widest camera keyframe (usually f0 / the loop
   hinge) and confirm no layer edge or background seam shows.
...
4. **QA** — `cd remotion && node scripts/qa_frames.mjs <Id> <entrance-cues,transitions,last> --scale=0.333`
   → small JPEGs; READ those. (Legacy `scripts/frames.mjs` writes full PNGs that flood context.)
