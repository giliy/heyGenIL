...
> main-session context, defer orchestration to the **`orchestrate`** skill. The collage contrast rule
> and layer-dissection discipline are baked into the pixel/build agent definitions. The stages below
> are the same work done inline when you are NOT using the router.
...
node scripts/qa_frames.mjs VoxNName <f0,entrance-cues,camera-arrivals,transitions,last> --scale=0.333
```
READ every small JPG: every entrance landed · camera framing at each arrival (nothing critical
...
Mux via `tools/ffw.py` (bare ffmpeg can silently produce a blank track) and gate:
`python tools/audio_gate.py <proj>/output/<...>-voiced.mp4`.
...
Library-first and **recorded clips before generated**: draw from `catalog.json` preferring
`source` = `sonniss-gdc-*`/`kenney`/`soundcn`/`freesound` (`pro-*`/`rec-*` ids); for a miss
import a recorded clip via `tools/import_sfx.py` (see `docs/sfx-sources.md`) before any
`gen_sfx.py` fallback. Sonniss `pro/` clips are gitignored — per-machine
`tools/fetch_pro_sfx.py`. The user's ear is the audit gate.
