...
> **Multi-agent mode:** to run this end-to-end through the pinned specialist agents with a small
> main-session context, defer orchestration to the **`orchestrate`** skill. The ai-shorts iron rules
> (cost stated before generating, character locked from character.json, loop pinned via end-frame
> conditioning) are baked into the pixel/build agent definitions and still apply. The stages below are
...
2. **Voice FIRST** — `python tools/gen_voice.py --beats ... --emit-ts remotion/src/shots/ai-N/vo.gen.ts`
   ...
   **Hebrew:** ElevenLabs Hebrew is ONLY `eleven_multilingual_v3 --lang he` (kokoro excluded);
   the default is the FREE edge-tts path — `python tools/gen_voice_edge.py --beats ... --voice
   he-IL-HilaNeural --rate -8% --pitch +4Hz` (motherese for kids; male alt `he-IL-AvriNeural`).
   Kids language rules: `research/hebrew-kids/07-hebrew-language-kids.md`; the locked character's
   `character.json` carries its voice persona.
...
6. **Composition** — Sequences of `OffthreadVideo` over ONE timeline, ~8-frame crossfade
   ...
   `media/projects/<name>/`. `npm run gen`, then the CONTEXT-SAFE QA renderer
   `node scripts/qa_frames.mjs <Id> <boundaries+heroes> --scale=0.333` (small JPEGs — READ these,
   not full PNGs), READ every frame, then render-all --scale=1 and mux voice.wav via
   `tools/ffw.py` (bare ffmpeg can silently produce a blank track) + `tools/audio_gate.py`.
7. **SFX** — /suggest-sfx flow (recorded-first: prefer `catalog.json` clips with `source` =
   `sonniss-gdc-*`/`kenney`/`soundcn`/`freesound` — `pro-*`/`rec-*` — import misses via
   `tools/import_sfx.py`, Sonniss `pro/` is gitignored + fetched per machine via
   `tools/fetch_pro_sfx.py`; see `docs/sfx-sources.md`). **Gappy-VO calibration (blue-man #1):** cues in VO gaps get no
