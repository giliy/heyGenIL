...
4. **Library-first sourcing — RECORDED clips before generated.** For each distinct sound the plan
   needs, reuse a catalog clip if one fits — and **prefer a recorded clip** (`source` =
   `sonniss-gdc-*` / `kenney` / `soundcn` / `freesound`) over an ElevenLabs-generated one for the
   same job; recorded reads premium where AI text-to-sfx reads synthetic. For misses, FIRST try to
   import a recorded clip (`python tools/import_sfx.py`, manifest + staging dir — see
   `docs/sfx-sources.md`); only if the library truly lacks the sound add a generic recipe to
   `palette.json` and generate via `python tools/gen_sfx.py` (ElevenLabs → normalized clip →
   catalog). Record `source` + `license` either way. License tiers: CC0/MIT clips commit to
   `clips/`; Sonniss/99Sounds clips live in the **gitignored** `pro/` dir (fetched per machine via
   `tools/fetch_pro_sfx.py`) — never commit their raw files.
...
7. **Mix the audition preview** once approved: `python tools/mix_sfx.py shorts/short-N-<niche>/sfx-plan.json`
   ...
   - **Verify audibility with numbers, not hope.** After mixing, RMS-diff the mixed audio vs the
     ...
     Use the gate: `python tools/audio_gate.py --mix <out> --base <voiced> --cue <t1> --cue <t2>`
     (add `--list-cues` to eyeball without enforcing). Mixing runs through `tools/ffw.py` (a full
     ffmpeg; the Remotion-bundled build silently breaks the duck chain).
