...
> **Multi-agent mode:** to run this end-to-end through the pinned specialist agents (story-writer →
> voice ∥ pixel → build → qa → mix) with a small main-session context, defer orchestration to the
> **`orchestrate`** skill — it encodes the spawn order, contract validation, and human gates. The
> stages below are the same work done inline (single-context) when you are NOT using the router.
...
# CONTEXT-SAFE QA (preferred): parallel small-JPEG renderer — READ these, not full PNGs.
node scripts/qa_frames.mjs ShortNName 0,<beat-boundaries+hero-frames>,<last> --scale=0.333
# (legacy, slow, reads big PNGs: node scripts/frames.mjs ShortNName ... --scale=0.5)
```
READ every small JPG (phone scale). Checklist: frame 0 composed & thumbnail-grade · every
reveal synced to its beat · nothing covering the payoff element · kickers swap with the
narration · captions clear of UI zones · loop frame ≈ frame 0. Fix, re-render frames, only then:
...
### Hebrew voice routing (language:"he")

ElevenLabs Hebrew exists ONLY on `eleven_multilingual_v3 --lang he` — v2/flash have NO Hebrew,
and kokoro is excluded. For Hebrew shorts the default is the FREE edge-tts path (native word
boundaries, no API key, battle-tested on `short-16-formy`):

```
python tools/gen_voice_edge.py --beats shorts/short-N-<niche>/beats.json \
    --voice he-IL-HilaNeural --rate -6% --pitch -2Hz \
    --emit-ts remotion/src/shots/short-N/vo.gen.ts
```
- **Female/warm default:** `he-IL-HilaNeural`. **Male alternative:** `he-IL-AvriNeural`.
- **Kids (mode:"kids"):** keep `he-IL-HilaNeural` but warm it up — `--rate -8% --pitch +4Hz`
  (motherese register: slightly slower, gentle rise). Language rules live in
  `research/hebrew-kids/07-hebrew-language-kids.md`; character voice persona in its character.json.
- `--rate`/`--pitch` are percent/Hz offsets; adjust per persona, then regen (cached by text-hash).
- ElevenLabs is the emotional-upgrade fallback only: `--model eleven_multilingual_v3 --lang he`.
- Then re-render the composition (captions retimed) and mux. ALWAYS go through the resolved
  ffmpeg wrapper (bare `ffmpeg` can resolve to the Remotion-bundled minimal build and produce a
  SILENT track — see the silent-voice incident): `python -c "import sys,os;sys.path.insert(0,'tools');import ffw;ffw.ffmpeg('-y','-i','remotion/out/<Id>.mp4','-i','shorts/.../voice/voice.wav','-map','0:v','-map','1:a','-c:v','copy','-c:a','aac','-b:a','192k','-ar','48000','-ac','2','-shortest','remotion/out/<Id>-voiced.mp4')"`
- **Then run the audio gate — a mux is not done until it passes:**
  `python tools/audio_gate.py remotion/out/<Id>-voiced.mp4` (fails loudly on a silent track).

## Stage 5 — SFX (the suggest-sfx machinery, shorts flavor)

Read `brand.md` §7 + `media/library/sfx/catalog.json`. Library-first, **recorded clips before
generated** — prefer catalog entries whose `source` is `sonniss-gdc-*`/`kenney`/`soundcn`/
`freesound` (ids `pro-*`/`rec-*`) over ElevenLabs-generated ones; for a miss, import a recorded
clip (`tools/import_sfx.py` — see `docs/sfx-sources.md`) and only then fall back to a
`palette.json` recipe + `python tools/gen_sfx.py`. Sonniss clips live in the gitignored `pro/`
dir — on a fresh machine run `tools/fetch_pro_sfx.py` first; the mixer warns-and-skips their
cues if absent. Author
